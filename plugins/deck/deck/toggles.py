...
from .config import ENV_PREFIX, STATE_DIR, core_root, die, load_yaml, norm, pack_dirs, session_id, task_file
...
class Toggles:
    ...
    def __init__(self, repo: str | None = None, session: str | None = None, root: Path | None = None):
        ...
        for pack in ws.all_packs() if ws else pack_dirs():
            ...
            if fragment:
                catalog = _merge_catalog(catalog, fragment, pack.name)
        ...
        self.choices = load_yaml(self.root / STATE_DIR / "toggles.yaml") if self.root else {}
