...
def cmd_cost(ws: Workspace, args) -> int:
    ...
    if wanted and not any(p.stem == wanted for p in paths):
        # It no longer falls back, and this line used to say it did. Naming a
        # session that is not this workspace's now comes back empty rather than
        # as the newest one in scope billed under somebody else's id — the
        # silent misattribution the scoping exists to stop. `--any-project`
        # widens the search when that is what somebody means.
        print(f"deck: no transcript named {wanted} in this workspace")
        print("  `--any-project` looks outside it; `deck propose lessons --session` reads one")
        print("  from anywhere, because a lesson is not a bill and does not have to be yours.")
    if not paths:
