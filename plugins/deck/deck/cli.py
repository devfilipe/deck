...
def _workspace_parsers(sub) -> None:
    ...
    # `deck scope <name>` reports one, and `deck scope <verb> ...` acts. Both,
    # because the report is what somebody types first and renaming it would
    # break every document that names it — so the verbs are recognised by their
    # own names and anything else is read as a scope to report on. A scope
    # actually called `new` or `select` is the price, and it is a small one.
    p = sub.add_parser("scope", help="one scope: report it, create it, or select it")
    p.add_argument("name", help="a scope to report on, or one of: new, add-repos, select")
    p.add_argument("rest", nargs="*", help="arguments for the verb")
    p.add_argument("--json", action="store_true")
    p.add_argument("--force", action="store_true")
    p.add_argument("--title")
    p.set_defaults(fn=cmd_workspace.cmd_scope_dispatch)
    ...
    p.add_argument(
        "--workspace", help="the workspace name the descriptor is versioned under (default: the root directory's name)"
    )
    p.add_argument(
        "--repos",
        nargs="*",
...
def _toggle_parsers(sub) -> None:
    ...
    p = inner.add_parser("unset", help="withdraw a recorded value")
    p.add_argument("id")
    p.add_argument("--at", default="task", metavar="task|workspace|<scope>", dest="write_at")
    p.set_defaults(tfn=cmd_toggle.cmd_unset)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p = sub.add_parser("save", help="carry the agent's edits back into the pack collection")
    p.add_argument("--task", help="task id; defaults to the session id")
    p.add_argument("--dry-run", action="store_true", help="what would be written, writing nothing")
    p.set_defaults(fn=cmd_mount.cmd_save)
    ...
    p.add_argument("--pr", action="store_true", help="report the state of the pull request this task's branch has")
    p.add_argument(
        "--open-pr",
        action="store_true",
        help="open or update that pull request with this bundle as its body",
    )
    p.add_argument("--repo", help="whose branch the request carries, when the task names more than one")
    p.add_argument("--yes", action="store_true", help="send the write without asking")
    p.add_argument("--json", action="store_true")
    ...
    p.add_argument(
        "--budget",
        type=float,
        default=assist_default(),
        help="loop cost cap in USD — the call already running when it trips still bills",
    )
    p.add_argument("--timeout", type=int, default=600, help="seconds to wait for the call (default 600)")
    ...
    p = propose_sub.add_parser("lessons", help="what this session taught, sorted into the kinds a pack holds")
    p.add_argument("--session", help="a session id; defaults to this workspace's most recent transcript")
    p.add_argument("--budget", type=float, default=assist_default())
    p.add_argument("--timeout", type=int, default=600, help="seconds to wait for the call (default 600)")
    p.add_argument("--yes", action="store_true", help="permit the call when ai_assist is `ask`")
    p.add_argument("--show-prompt", action="store_true", help="print what would be asked, and stop")
    p.set_defaults(fn=cmd_propose.cmd_propose_lessons)
    ...
    p.add_argument(
        ...
        action="store_true",
        help=(
            "required before an `impacts` proposal writes the shared descriptor every other "
            "command reads; a `pack` or `lessons` proposal writes into the one pack directory "
            "named by --into without it — reading the draft first is that confirmation"
        ),
    )
    ...
    p.add_argument(
        "--header", help="toggle: 12 characters or fewer, wording a selector would use if it were ever set to ask"
    )
