...
def cmd_ask_fold(ws: Workspace, args) -> int:
    ...
    if args.as_kind == "rule":
        ...
        # No second step: mount walks `rules/`, so a file written here is a file
        # that will be placed. There is no entry to forget.
        written = [f"rules/{name}.md"]
        rel = f"rules/{name}.md"
        ...
        if any(t.get("id") == tid for t in (existing.get("toggles") or [])):
            ...
            return 1
        # A person's answer starts a sentence with a capital, and the first word
        # of "which of these two" is often the value itself — fold case here so
        # that ordinary prose still resolves, without loosening `norm` itself,
        # which other call sites use for identity comparisons where case is
        # meaningful.
        answer_folded = norm(entry["answer"]).casefold()
        default = args.default or next((v for v in values if norm(v).casefold() in answer_folded), None)
        if default is None:
            return _refuse(
                [
