...
from .config import core_root, dump_yaml, load_yaml, run, slug, STATE_DIR, norm
from .workspace import Workspace, board_identity, person_fields
...
# machine-specific field in the list and is not. The one per-machine fact a
# descriptor holds is where the workspace root is, and no repository entry
# holds it: `deck init` resolves the root, and every `path:` is written
# relative to it. `services/api-server` is the layout everyone clones, so
# `path` travels for exactly the same reason `impacts:` does.
#
...
# allowlist goes.
#
# `scopes` passes it, and was missing: how a product is carved into initiatives
# is the team's answer, not one machine's, and the template is the only place
# it can be versioned. Left out, the one documented route to making a scope the
# team's dropped it in silence, and `deck doctor` could then report a scope as
# local to this machine straight after the command meant to publish it ran.
SEEDED_WORKSPACE_FIELDS = ("sources", "container", "requires_files", "scopes")
...
MOUNT = """# What deck places in a repository when this pack is in play.
#
# Order of preference is the engine's, not yours: the plugin entry keeps the
# content in one versioned place and costs the repository three lines of local
# settings; a rule is symlinked, so `ls -l` shows where it came from; copying a
# file into a product repository is the last resort.
#
# Everything placed is recorded in a manifest and removed on unmount. deck never
# deletes what it did not place.
...
# `paths:`-scoped rules from this pack's rules/ directory.
# Entries go under the key below, indented as the example is. Uncomment it and
# edit, or delete it and write your own.
rules:
...
def cmd_pack_new(ws: Workspace, args) -> int:
    name = args.name
    target = Path(args.dir) if args.dir else Path.cwd() / "packs" / name
    ...
    files = {
        ...
        "rules/example.md": RULE_EXAMPLE,
        "skills/example-procedure/SKILL.md": SKILL_EXAMPLE,
        "agents/README.md": AGENTS_NOTE,
    ...
    if bind:
        ...
        seed["repos"] = {
            name: {k: v for k, v in entry.items() if k in SEEDED_REPO_FIELDS} for name, entry in ws.repos.items()
        }
        seed["targets"] = []
...
def cmd_pack_validate(ws: Workspace, args) -> int:
    ...
    mount = load_yaml(target / "config" / "mount.yaml")
    for rule in mount.get("rules") or []:
        if not (target / rule.get("file", "")).is_file():
            problems.append(f"mount.yaml names a rule that does not exist: {rule.get('file')}")
...
def _gate_history(root: Path) -> dict[str, dict]:
    ...
    stats: dict[str, dict] = {}
    for record in sorted((root / STATE_DIR / "gates").glob("*.json")):
        try:
...
def cmd_pack_review(ws: Workspace, args) -> int:
    ...
    for pack in packs:
        ...
        for rule in mount.get("rules") or []:
            rel = norm(rule.get("file", ""))
            path = pack / rel
            if not path.is_file():
                findings.append(f"rule {rel} is declared and missing")
                continue
            patterns = _front_matter(path).get("paths") or []
            if not patterns:
                continue
            targets = [r for r in (rule.get("repos") or repos) if r in ws.repos]
            reached = False
            for name in targets:
