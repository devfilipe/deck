...
from .config import ENV_PREFIX, publish_identity, state_root
...
def _mark(task: dict) -> str:
    """The one character that says whether a task is open to be taken.

    `[ ]` for everything not closed is how a claim disappeared: `board claim`
    writes `in-progress` and an assignee, and the board printed a moment later
    read exactly like a task nobody had taken — so the next person to look was
    told it was free.

    `~` is `in-progress`, and nothing else. An assignee is not read as one: a
    tracker has two states, so `in-progress` never comes back from it, and the
    obvious repair — treat anyone assigned as working on it — is wrong for every
    team that assigns before anybody starts, which would have its whole board
    read as taken. What the source can say about that, the source says, in
    `in_progress:` (see `trackers.in_progress_from`); where it says nothing,
    `board list` prints the holder's name and says once that `[ ]` here means
    "not closed" rather than "nobody is on this".
    """
    return {"done": "x", "in-progress": "~"}.get(task.get("status"), " ")


def _staleness(task: dict) -> str:
    """How this record arrived, when it did not arrive from the board.

    Per task and not only per source: a board read from two sources, one of them
    live, otherwise offers no way to tell which half is a memory.
    """
    if not task.get("stale"):
        return ""
    return f"   from a cache, {board_lib.age(time.time() - float(task.get('cached_at') or 0))} old"
...
def cmd_board_list(ws: Workspace, args) -> int:
    ...
    for task in tasks:
        ...
        # github numbers issues and pull requests in one sequence, so `#11` does
        # not say which arrived — and a board that shows work in review as work
        # not started is showing half of what is happening.
        what = task.get("kind")
        print(f"  [{mark}] {task['id']:<24} {task['title'][:60]}")
        print(f"        {f'{what} · ' if what and what != 'issue' else ''}{repos}{_staleness(task)}")
        if task.get("assignee"):
            print(f"        held by {task['assignee']}")
    for problem in problems:
        print(f"  ! {problem}")
...
def cmd_board_plan(ws: Workspace, args) -> int:
    ...
    for bucket in payload["groups"]:
        ...
        for task in bucket["tasks"]:
            # A name the registry does not hold is dropped from the closure, and
            # "(no repository named)" then said the opposite of what the board
            # says — the task named one, deck could not place it. Saying which
            # name is what turns the refusal below into something actionable.
            unplaced = [r for r in task["repos"] if r not in task["reaches"]]
            reaches = ", ".join(task["order"]) or (
                f"(named {', '.join(unplaced)} — not in the registry)" if unplaced else "(no repository named)"
            )
            print(f"    [{_mark(task)}] {task['id']:<24} {task['title'][:52]}")
            if task.get("assignee"):
                print(f"      held by: {task['assignee']}")
            print(f"      reaches: {reaches}")
            if task["target"]:
                print(f"      target:  {task['target']}")
            # The earliest group that turned this task away, and why. Only the
            # first: on a board where every task names the one repository each
            # task is refused by every group before it, and printing all of them
            # buries the one line that answers "why is this alone" under n more
            # that say it again. The whole list is in `--json`.
            for entry in (task.get("held_from") or [])[:1]:
                against = f" ({entry['against']})" if entry["against"] else ""
                print(f"      not with group {entry['group']}{against}: {entry['reason']}")
        print()
...
def writes_allowed(ws: Workspace, what: str, args) -> bool:
    """Writing to a tracker is outward-facing, so it is a decision, not a default.

    Not private any more: `deck bundle --open-pr` writes to the same host under
    the same permission, and a second copy of this rule is a second place for it
    to drift away from the toggle that is supposed to govern it.
    """
    tg = Toggles(root=ws.root)
...
def cmd_board_show(ws: Workspace, args) -> int:
    ...
    if task.get("kind") and task["kind"] != "issue":
        print(f"  kind      {task['kind']}")
    if task.get("stale"):
        old_by = board_lib.age(time.time() - float(task.get("cached_at") or 0))
        print(f"  read      from a cache {old_by} old — this is the board as it was, not as it is")
...
def _refuse_stale(task: dict, what: str) -> bool:
    """A write against a board deck could not read is refused, and says why.

    The cache exists so an outage does not hide the board; it is not a licence
    to write against a memory of it. Two people taking one task is exactly what
    a claim is for preventing, and a claim decided by a record that may be days
    old prevents nothing.
    """
    if not task.get("stale"):
        return False
    old = board_lib.age(time.time() - float(task.get("cached_at") or 0))
    print(f"deck: {task['id']} was read from a cache {old} old, not from the board.")
    print(f"  {what} against a board deck could not read is how two people take one task.")
    print("  Fix the connection and run it again; `deck board list` says what it could reach.")
    return True
...
def cmd_board_claim(ws: Workspace, args) -> int:
    ...
    if task is None:
        print(f"deck: no task named {args.id}")
        return 1
    # Before `source is None`, which a cached read reaches too — the source
    # lookup fetches, and a fetch that fails holds no task. "Not in a source
    # deck can write to" would be a true sentence about the wrong problem.
    if _refuse_stale(task, "A claim"):
        return 1
    ...
    if not writes_allowed(ws, f"claiming {args.id} for {who}", args):
        return 1
...
def cmd_board_done(ws: Workspace, args) -> int:
    ...
    if task.get("status") == "done":
        ...
        return 0
    if _refuse_stale(task, "Closing a task"):
        return 1

    evidence = state_root(ws.root) / "gates" / f"{args.id}.json"
    ...
    kind = board_lib.norm(source.get("type", ""))
    if not writes_allowed(ws, f"closing {args.id}", args):
        return 1
    ...
    if kind in ("tasks", "board"):
        ...
        if board_lib.update_local(ws, path, args.id, changes):
            ...
            if not criteria:
                # The guard above is as strict as the source allows, and a
                # source that carries nothing makes it silent. A tracker has no
                # field for acceptance criteria and deck does not read them out
                # of an issue body, so a board moved onto one loses this half of
                # the definition of done — and used to lose it without a word,
                # which is the failure the guard exists to prevent.
                print("  It declares no acceptance criteria, so this closure rests on the ladder alone:")
                print("  nothing here states what the task was for. `deck board template` has the shape.")
                if task.get("ext_provider"):
                    print(f"  {task['ext_provider']} has no field for them either — only this entry can carry them.")
            if task.get("ext_provider") and task.get("ext_id"):
...
def cmd_board_new(ws: Workspace, args) -> int:
    ...
    if not writes_allowed(ws, f"creating a task in the {kind} source", args):
        return 1
