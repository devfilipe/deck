...
from .config import load_yaml, state_root
...
def transcripts(session: str | None = None, root: Path | None = None, anywhere: bool = False) -> list[Path]:
    ...
    if session:
        ...
        if exact:
            return exact
        # Named, and not in scope. What happens next differs by CALLER, and
        # the difference is not a detail:
        #
        #   `deck cost` keeps the scope. A session belonging to another project
        #   is not this workspace's bill, however explicitly it was named, and
        #   attributing it here is the silent error the scoping exists to stop.
        #
        #   `deck propose lessons` passes `anywhere=True`. A session that works
        #   a workspace from OUTSIDE it — an agent running in a tools checkout,
        #   a session opened in the collection — writes its transcript under
        #   its own directory. It is invisible to the scope and is exactly the
        #   session somebody means. Naming an id is not guessing.
        #
        # Either way it RETURNS EMPTY rather than falling through. Falling
        # through handed back the newest transcript in scope under a different
        # id, which is a wrong answer wearing a right one.
        if not anywhere:
            return []
        return sorted(
            (f for d in base.iterdir() if d.is_dir() for f in d.rglob("*.jsonl") if f.stem == session),
            key=lambda p: p.stat().st_mtime,
            reverse=True,
        )
    return files
...
def window_for_task(root: Path, task: str) -> tuple[datetime | None, datetime | None, str]:
    ...
    # deletes by design.
    span = state_root(root) / "state" / "tasks" / f"{task}.json"
    if span.is_file():
        try:
    ...
    manifest = state_root(root) / "mounts" / f"{task}.json"
    ...
    evidence = state_root(root) / "gates" / f"{task}.json"
...
def samples_path(root: Path) -> Path:
    return state_root(root) / "cost" / "samples.jsonl"
