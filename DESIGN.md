...
| `mount` | How does a pack get into a working directory, and back out? | Plugin entry where a plugin will do; otherwise a copied file, recorded in a manifest with its hash |
...
`deck toggle unset <id> --at <layer>` takes one back — refusing when that exact
layer has nothing recorded, rather than silently succeeding, and printing which
value becomes effective afterward and where it comes from, the same sentence
`explain` already knows how to produce. It does not take `--why`: `reasons:` is
paired 1:1 with `values:` (a reason with no value beside it is itself something
`toggle validate` refuses), so there is nowhere in the current file for a
reason to withdraw something to live — the reason it withdraws is printed at
the moment of the act instead. Without it, trying a value at a narrow layer and
taking it back to see the wider one resurface meant hand-editing two maps in
step, which is exactly the kind of drift this file is built to avoid.
...
```
   ...
   rules/api-contract.md      ──copy────►     .claude/rules/deck-api-contract.md
...
Copying is the mechanism, not a last resort, and the reason is the runtime:
Claude Code does not follow a symlink in `.claude/rules/`, so a mount that
linked would report success while placing something nothing reads. The cost of
copying is drift, and `deck save` is what pays it — it compares each placed file
against its source and carries an edit back to the pack. `unmount` asks the
other question, against the hash recorded at mount time, and leaves an edited
file alone rather than deleting it.
...
|---|---|---|
| the descriptor | the team | Yes — versioned in the collection, reviewed like code |
| `~/.deck/workspaces/<name>/machine.yaml` | the machine | No — paths and targets differ per person |
| `~/.deck/workspaces/<name>/toggles.yaml` | the machine | No — a template is versioned in the pack |
...
and exercises the observable behaviour, covering resolution,
