...
|---|---|
| **workspace** | the tree deck manages, holding one or more repositories. Named by `.deck/workspace.yaml` |
...
| Container / toolchain | `container` in the descriptor, used by gate commands | project | yes | works |
...
|---|---|
| `.deck/workspace.yaml`, `.deck/toggles.yaml` | configuration you then edit |
| a pack skeleton | static template files, copied |
| a mounted rule | a symlink into the pack |
...
|---|---|
| **A scope is per machine** | `scopes:` lives in `.deck/workspace.yaml`, which is not versioned, so how a product is carved into initiatives is not shared by the descriptor itself. A pack's `templates/workspace/workspace.yaml` ships the carve-up — `deck pack new --from-workspace` writes it there, and `deck doctor`, `deck scopes` and `deck scope <name>` name every way this machine's copy has diverged from it: both subsets spelled out, and which of the two deck is using. What is left is the half a comparison cannot reach. A team that ships no template has nothing to be measured against, and a divergence is still reported rather than resolved — which of the two sides is right is not something a tool can know. |
