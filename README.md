...
**Em português:** [um guia completo](https://devfilipe.github.io/deck/pt-br.html)
— o problema, o vocabulário, cada comando com suas opções, e os fluxos de uso
com diagramas.
...
```
   projs/                          ai-packs/
   ├── group1/                     ├── _workspaces/all/default/  ← every repository
   │   ├── proj1/   ────────────►  ├── _repos/proj1/
   │   └── proj2/   ────────────►  ├── _repos/group1/proj2/  ← groups may be mirrored
   └── group2/proj3/────────────►  └── _repos/group2/proj3/
```
...
deck toggle unset gate_level --at workspace  # withdraw it; the wider layer resurfaces
...
deck propose apply impacts.json --yes                 # writes the shared descriptor
deck propose apply pack.json --into <pack dir>         # writes into one pack's own directory
```
...
`apply` does not ask the same way for every kind of proposal it takes. An
`impacts` draft edits `workspace.yaml` — the descriptor every other command
reads, whose edges decide execution order and whether the graph still has one
— so it always prints a preview first and writes only with `--yes`. A `pack` or
`lessons` draft only appends into the one pack directory named by `--into`:
gates and rules that do nothing until `deck mount` and `deck gate run` next
touch them, skills and agents Claude Code does not load until they are
mounted, consultations that are questions recorded, not decisions taken. That
write always happens, `--yes` or not — reading the draft before pointing
`--into` at a pack you mean is the confirmation, and there is no second one to
type.
...
```bash
./ci/smoke.sh    # every command, against a synthetic workspace; touches nothing of yours
```
