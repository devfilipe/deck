...
│   ├── mount.yaml              what gets placed in a repository, and how
│   └── profiles.yaml           named postures
│
├── rules/                   ── deck places these; Claude Code loads them ──
│   └── <name>.md               `paths:`-scoped rules, symlinked in on mount
│
├── templates/workspace/
...
### `mount.yaml` — what lands in a repository
...
Preference order is the engine's: the plugin entry keeps content in one
versioned place; a rule is symlinked so its origin is visible; copying a file
into a product repository is the last resort. Everything placed is recorded in a
manifest and removed on unmount, and deck never deletes what it did not place.
...
A profile is the base layer. Anything in `.deck/toggles.yaml`, in the
...
| Travels | Why |
|---|---|
| `path` | relative to the workspace root, and the root is the only per-machine fact a descriptor holds — no entry holds it |
...
silently into a file a team is asked to review.
...
Three fields have been lost by omission already — `couples`; `downstream`, which
meant every repository that produces no artifact came out of a seed looking like
an ordinary build node; and `scopes:`, which meant the one documented route to
making a carve-up the team's dropped it in silence.
...
A board is declared under `backlog:` in `.deck/workspace.yaml`. deck reads four
...
above. A pack ships the carve-up in `templates/workspace/workspace.yaml` —
`deck pack new --from-workspace` puts it there along with the repositories. That
is what makes a scope the team's rather than one person's: the descriptor it
lands in is per machine, so a scope nobody templated is a scope only its author
has.

#### Where an initiative's definition lives

**The registry is not merged. The scopes are.** Which repositories exist and how
a change propagates between them is a description of a thing that exists: one
answer serves everybody, and a second opinion about it is a contradiction. So
`repos:` comes from one template, the most specific pack's, exactly as before. A
scope is a decision about how to divide attention, several can be true at once,
and merging decisions is ordinary — so deck reads `scopes:` from **every** pack
and puts them together.

That is what lets an initiative be versioned apart from the registry and from
other initiatives. A scope-bound pack ships the scope it is bound to, in a
template with no `repos:` block at all:

```yaml
# packs/payments-ops/templates/workspace/workspace.yaml
scopes:
  payments:
    title: Payments migration
    repos: [api-schema, api-server]
...
```bash
deck pack new payments-ops --scope payments --from-workspace
```

writes both halves: `scope: payments` in `config/detect.yaml`, and that block in
the template. Two initiatives are then two packs with two owners and two
reviews, and neither team edits the file the other one lives in. A scope-bound
pack is never read for the registry, whichever scope is active — `deck init` and
`deck setup` seed the repositories from the workspace pack's template and skip
this one.

**A name two packs both ship is reported, never merged.** It is either a mistake
or a deliberate override, and deck does not rank two answers to one question:

```
$ deck doctor
scopes
  XX scope  scope `payments` is shipped by more than one pack (_workspace, payments-ops) —
            deck does not rank two answers to one question, so this machine's carve-up is
            compared against neither. Keep `payments` in one of them and drop it from the rest
```

The divergence report below names, per scope, the pack that ships it — not one
pack for the whole file, because with the definitions merged there is no longer
one answer to "which pack do I go and edit".

**And the copy on this machine is checked against it.** Two people running the
same command in the same named scope over different repositories is a failure
that is quiet by construction — neither of them is told. `deck doctor`, `deck
scopes` and `deck scope <name>` compare the two and report four different facts
at two volumes:

| What differs | Voice |
|---|---|
| The same name holding a different subset | **warning** — both subsets named, and which one deck is using |
| The same subset reading a different `backlog:` | **warning** — same repositories, different tasks |
| A scope a pack ships and this machine does not declare | **warning** — naming the pack that ships it, with the block to copy and where from |
| A scope only this machine declares | a note. A local initiative is a legitimate thing to have |
| The same subset under a different `title:` | a note. It changes nothing any command does |

Repositories are compared as a set: two people who wrote the same six names in a
different order are working on the same initiative.

None of it changes an exit code, and none of it stops a command. deck keeps
working on both sides of a divergence, and the report says which side is in
force, because which one is *right* is not something a tool can know. A pack
that ships no `scopes:` is not accused of anything — there is nothing to
disagree with.
...
backlog:
  - type: roadmap
    ...
    command: packs/_workspace/bin/roadmap-to-tasks.py roadmap planning/roadmap.md
```
...
```
   ...
   └── group2/                     └── group2/proj3/
       ├── proj3/   ────────────►
       └── proj4/                  (no pack — deck says so rather than staying quiet)
```

Point deck at one or more collections with `packs_root:` in the descriptor
or `DECK_PACKS_ROOT`, and it matches a pack directory to a repository by the
directory name or the registry name, looking two levels deep so a collection can
mirror your groups. `_workspace` (also `_all`, `_shared`, `_common`) applies to every
repository — which is where a suite-wide pack belongs.
...
| **Development** | `packs/_workspace/` | Claude, in deck's own checkout | a pack, mounted while contributing |
...
```
   ...
   _workspace           every repository, by its name
          ↓
