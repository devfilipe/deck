...
```
   ...
   └── group2/                     └── group2/proj3/
       ├── proj3/   ────────────►
       └── proj4/                  (no pack yet — deck says so)
```

deck looks two levels deep and matches on the directory name or the registry
name. `_workspace` (also `_all`, `_shared`, `_common`) applies to every repository — that is
where a suite-wide pack belongs. A name claimed by two packs is reported rather
than resolved; picking one would be a guess about your intent.
...
`.deck/workspace.yaml`, which is yours and not versioned, says where yours is.
...
3  Writing the descriptor          .deck/workspace.yaml · .deck/toggles.yaml
...
Which repositories a change forces you to touch. By hand in
`.deck/workspace.yaml`, or ask for a draft:
...
Open `~/work/ai-packs/_workspace/config/gates.yaml`. This is where your build,
...
4  Linking packs to repositories, by name
   linked   proj1     ~/work/ai-packs/proj1
   linked   proj2     ~/work/ai-packs/group1/proj2
   no pack  proj4     create ~/work/ai-packs/proj4
   shared   (every repository)   ~/work/ai-packs/_workspace
```
...
```bash
$EDITOR .deck/workspace.yaml     # `targets:` — the machines you may reach
...
| `.deck/workspace.yaml` | this machine | no — paths, hosts, tokens |
| `.deck/toggles.yaml` | this machine | no — your own posture |
