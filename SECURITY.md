...
| **Hosts** | `targets:` is an allowlist over the target a gate may name: `deck gate run` refuses a `target` this workspace has not declared, before the command is built. It is not a network sandbox — a command that writes a hostname literally is not intercepted, and what limits what an agent may execute is Claude Code's permission surface |
