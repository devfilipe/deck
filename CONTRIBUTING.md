...
cd deck
cp seed/workspace.yaml .deck/workspace.yaml     # .deck/ is per machine
...
`deck doctor` warns about this; `packs/_workspace/rules/self-hosting.md` gives
the clone commands.
...
`packs/_workspace/config/gates.yaml`. A gate added to one and not the other
...
anywhere and touches nothing you own.
